...
Establish Agora as the Knowledge Islands name and profile contract for opening one deliberately bounded Zed workspace per named project through the `ki` CLI.
...
- **Contract:** A named `[projects]` table, a required `primary` key, primary-first ordering, and a separate Zed workspace per project.
...
- [x] Open every project with `zed -n` in stable primary-first key order.
...
`primary` identifies the project opened first. Every project opens with `zed -n`, so it receives a separate Zed workspace; remaining projects follow in English lexical key order. Profile project keys are names, not an array, so `primary` can refer to one unambiguously.

### Phase boundary

Phase 1 is complete when the CLI loads an applied profile and each named project appears as a separate Zed workspace in visual acceptance. The prior `zed -a` implementation instead created one multi-root workspace, which does not meet this contract. Phase 2 may add macOS and Finder `.ki-agora` file association for double-click opening; it requires separate design and implementation work.
...
The active CLI has been source-linked and verified for `list` and `show`. Automated tests verify the profile contract, primary-first invocation, one new Zed workspace per project, stable remaining-project order, and rejection of the retired `cockpit` command.
