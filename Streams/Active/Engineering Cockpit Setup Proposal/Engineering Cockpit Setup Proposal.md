...
title: Engineering Cockpit Setup Proposal
status: draft
priority: medium
dependencies: []
---

# Engineering Cockpit Setup Proposal

## Overview

Establish an engineering cockpit for orienting, navigating, editing, reviewing, and coordinating AI-assisted work across the engineering estate.

The cockpit combines a live operations surface for concurrent agent and terminal work with a focused engineering interface for navigation, editing, and review. The work will configure and validate that practical arrangement in the existing production environment, preserving productivity throughout the transition.

The aim is a coherent cockpit practice, not a narrow editor-for-editor substitution. Zed, VS Code, Ghostty or Warp, Codex, Claude Code, Git, and Herdr retain distinct, complementary roles.
...
- **Decision:** Live, concurrent agent and terminal work requires a cockpit surface beyond Zed's single active agent panel.
- **Decision:** Techne distinguishes architectural components from replaceable implementation choices.
- **Decision:** Cockpit rosters are chezmoi-managed configuration and are opened through the `ki cockpit` command surface; Zed is the initial renderer.

## Outputs

- **Decision:** A defined operating boundary for Ghostty or Warp, Zed, VS Code, Claude Code, Codex, Git, Herdr, and Knowledge Islands within the engineering cockpit.
- **Artefact:** A configured and validated cockpit for Techne authoring, architecture, and documentation work.
...
- [ ] Define the initial cockpit workflows and operating boundaries for Ghostty or Warp, Zed, VS Code, Claude Code, Codex, Git, Herdr, and Knowledge Islands.
...
- [ ] Define when interactive agent work remains in the cockpit and when it becomes persistent work hosted by Herdr.
- [ ] Validate the cockpit on representative Techne work and record any constraints or follow-up configuration.
...
- [ ] Define the chezmoi-managed cockpit schema and initial named rosters for use by `ki cockpit`.
...
- Which Techne repositories and worktrees should form the initial cockpit workspace?
...
The engineering cockpit is the working environment through which the engineer sees and steers concurrent work.
...
The cockpit does not erase the distinct roles of other tools. Claude Code and Codex remain available for AI-assisted work, Git remains the source of versioned truth, and VS Code remains available where it is the better tool for a task.
...
- **Codex and Claude Code:** Remain AI collaborators for reasoning, planning, implementation, and review; their work is inspected and reconciled from the cockpit.
...
Named cockpits are managed through `ki cockpit`, not through a separate executable. Chezmoi renders their declarative definitions beneath `~/workspaces/ki-cockpits/`, analogous to the existing managed VS Code workspace files.

A cockpit definition names a purpose and lists independent project roots. The initial renderer is Zed: it opens one window and adds each root as a separate project. The definition remains tool-neutral so future renderers can serve Ghostty, Warp, or another chosen surface.

The initial cockpit-definition filenames are `ki-fnd.toml` for Knowledge Islands Fundamentals, `ki-mcps.toml` for MCP Development, `personal.toml`, `hnr.toml`, and `legal.toml`. Personal, HNR, and legal cockpits will be added only with deliberately bounded project rosters; HNR is distinct from other work domains.
...
Preserve the user's existing configuration unless a concrete cockpit workflow requires a change. Keep agent scope to a named repository or deliberately chosen multi-root workspace; do not use a broad parent directory as an agent's default working scope. Record each material choice and verify it against representative Techne work before treating it as established practice.
...
At initial inspection, `session.trust_all_worktrees` was `true`. The cockpit setup chose to restore Zed's secure default and trust known project roots deliberately.
